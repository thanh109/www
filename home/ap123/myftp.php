 ...
 */
     public function download($remotepath, $localpath, $mode = 'auto')
     {
        ...
        $mode = FTP_BINARY; //($mode == 'ascii') ? FTP_ASCII : FTP_BINARY;
 		$path = $this->getpath($localpath);//substr($localpath,0,strrpos($localpath,"/"));

        if (is_null($path) === false) //如果保存目录为空
		{ 
			if (is_dir($path) === false) 
			{ 
				$bolRes = mkdir($path, 0777, true); 
			} 
			chdir($path); 
		}
	...
	*/ 
	public function ftp_download_directory($strSrcDirectoryName, $strDestDirectoryName = null) 
	{
		//FTP连接
	    if( ! $this->_isconn()) {
	       return FALSE;
		...
		{ 
			if (@ftp_chdir($this->conn_id, $strSrcDirectoryName) === false)
			{ 
				//是文件，直接拷贝 
				$bolRes = ftp_get($this->conn_id, $strSrcDirectoryName, $strSrcDirectoryName, FTP_BINARY); 
				if ($bolRes === false) 
				{ 
					return false; 
				} 
		...
		$arrChildDirectory = ftp_nlist($this->conn_id, '.'); 
		/** 空目录 直接返回上层目录 */ 
		...
		{ 
			...
			//如果是目录， 进行递归 
			if (@ftp_chdir($this->conn_id, $strChildDirectory) === true) 
			{ 
				ftp_chdir($this->conn_id, '..'); 
				if($this->ftp_download_directory($strChildDirectory) === false)
				{
					 return false; 
				}
			} 
			...
			{ 
				...
				//echo "ftp currecnt directory ".ftp_pwd($this->conn_id)."\n"; 
				$bolRes = ftp_get($this->conn_id, $strChildDirectory, $strChildDirectory, FTP_BINARY); 
				if ($bolRes === false) 
				{ 
					return false; 
				} 
